...
import sys_attackvectorstable from "../../data/sys-attack-vectors.json"
import sys_attackexamplestable from "../../data/sys-attack-references.json"
import model_attackvectorstable from "../../data/model-attack-vectors.json"
import model_attackexamplestable from "../../data/model-attack-references.json"
...
function Taxonomy({ data, type }) {
    ...
    const attackvectorstable = type === "system" ? sys_attackvectorstable : model_attackvectorstable;
    const attackexamplestable = type === "system" ? sys_attackexamplestable : model_attackexamplestable;
    ...
    const onPressHandler_examples = attack => {
        ...
        if (foundExamples.length !== 0) {
            ...
            for (var i = 0; i < finalExamples.length; i++) {
                ...
                if (finalExamples[i].vectors) {
                    finalExamples[i].vectors.forEach((e) => {
                        var match = attackvectorstable.find(element => element.avId === e.avId)
                        if (match) {
                            chipsList.push(<Tooltip title={match.avName} arrow placement="top"><Chip label={e.avId} clickable size="small" color="default" onClick={() => setSelectedAttack(e.avId)} /></Tooltip>)
                        }
                    })
                }

                // chipsList.push(<Chip label={"abc"} size="small" color="primary" />)
                // console.log(finalExamples[i])
                examplesList.push(<li key={i}><a href={finalExamples[i].link} target="_blank" rel="noreferrer">{finalExamples[i].title}</a>&nbsp;{chipsList}</li>);
                // summary
                summaryList.push(<li key={i}>{finalExamples[i].summary}</li>);
                chipsList = []
            }
            setExText(examplesList);
            setSummaryText(summaryList)
        }
    }
...
//             textBuilderInherited.push(<span class="featuredTitle">Tool summary<span className="featuredSub" id='av-safeguards'><ol><li>This attack method is capable of hijacking multi-rotor drones using flying adversarial patches, or small images strategically positioned in the surroundings. As a result, the drone deviates from its intended trajectory, losing track of the target human and instead following the adversarial patch.
// </li></ol></span></span>)
    ...
    useEffect(() => {
        ...
        if (selectedAttack === "") {
            ...
            if (selectedAttack !== avFromLink) {
                ...
                navigate('/tree/' + type + '?' + currentUrlParams.toString())
            }
        }
        ...
        function update(source) {
            ...
            function singleclick(d) {
                ...
                if (!d.target.__data__.selected) {
                    selectedNodes.push(d.target.__data__);
                }
    ...
    return (
        ...
        <div className="taxonomy" id='taxonomyDiv'>
            ...
            <Box sx={{
                ...
                <Box sx={{}}>
                    <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                        ...
                        {type === "model" ? <a href="https://github.com/pralab/secml-torch" target="_blank"><Tab label="SecML" /></a> : null}
                    </Tabs>
                </Box>
                <TabPanel value={value} index={0}>
                    {selectedSafeguard.length === 0 ? <AttackSearchbar selectedAttack={setSelectedAttack} disable={false} fromUrl={avFromLink} attacktype={type} /> : <AttackSearchbar selectedAttack={setSelectedAttack} disable={true} attacktype={type} />}
            ...
            <div className="featured" id="featuredContainer">
                ...
                <div className="featuredItem">
                    <span className="featuredTitle">References (Papers and Tools)</span>
                    <br></br>
